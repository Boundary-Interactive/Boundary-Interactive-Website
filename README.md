...
Launch copy, store links, team info, press kit assets, and social URLs live in `src/data/site.ts`. The canonical domain and contact emails live in `site.config.mjs` (`studioContactEmail` for general studio inquiries; `gameContactEmail` for Fly Exterminator support and press). Update `game.status`, `game.releaseWindow`, `links.*`, and `pressKitAssets` in `site.ts` when anything changes for the store or marketing.
